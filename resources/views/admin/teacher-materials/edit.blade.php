...
@section('page-title', __('messages.edit_material'))

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.teacher-materials.update', $teacherMaterial) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $teacherMaterial->title) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="title_ar" value="{{ old('title_ar', $teacherMaterial->title_ar) }}" dir="rtl"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">{{ old('description', $teacherMaterial->description) }}</textarea>
                </div>
                <div>
                    ...
                    <textarea name="description_ar" rows="3" dir="rtl"
                              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">{{ old('description_ar', $teacherMaterial->description_ar) }}</textarea>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="material_type" required
                        ...
                        @foreach(\App\Models\TeacherMaterial::allTypes() as $t)
                            <option value="{{ $t }}" {{ old('material_type', $teacherMaterial->material_type) === $t ? 'selected':'' }}>{{ strtoupper($t) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="language"
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                        @foreach(['english'=>'English','arabic'=>'Arabic / عربي','bilingual'=>'Bilingual','filipino'=>'Filipino'] as $val=>$lbl)
                            <option value="{{ $val }}" {{ old('language', $teacherMaterial->language)===$val ? 'selected':'' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="status" required
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                        <option value="active" {{ old('status', $teacherMaterial->status)==='active' ? 'selected':'' }}>{{ __('messages.active') }}</option>
                        <option value="draft"  {{ old('status', $teacherMaterial->status)==='draft'  ? 'selected':'' }}>{{ __('messages.draft') }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="text" name="subject" value="{{ old('subject', $teacherMaterial->subject) }}"
                           placeholder="e.g. Mathematics"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.grade_level') }}</label>
                    <input type="text" name="grade_level" value="{{ old('grade_level', $teacherMaterial->grade_level) }}"
                ...
                <div>
                    ...
                    <select name="category_id"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $teacherMaterial->category_id)==$cat->id ? 'selected':'' }}>{{ $cat->name_localized }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="source" value="{{ old('source', $teacherMaterial->source) }}"
                ...
                <div>
                    ...
                    <input type="number" name="published_year" value="{{ old('published_year', $teacherMaterial->published_year) }}"
            ...
            <div>
                ...
                <input type="text" name="tags"
                       value="{{ old('tags', is_array($teacherMaterial->tags) ? implode(', ', $teacherMaterial->tags) : '') }}"
            ...
            <div>
                ...
                <input type="url" name="external_url" value="{{ old('external_url', $teacherMaterial->external_url) }}"
                       placeholder="https://…"
                       class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    @if($teacherMaterial->file_path)
                    <p class="text-xs text-emerald-600 mb-1">✓ {{ __('messages.file_uploaded') }}: {{ basename($teacherMaterial->file_path) }}</p>
                    @endif
                    <input type="file" name="file_path"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                    <p class="text-xs text-gray-400 mt-1">{{ __('messages.leave_blank_keep') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.cover_image') }}</label>
                    @if($teacherMaterial->cover_image)
                    <img src="{{ $teacherMaterial->cover_url }}" class="w-16 h-16 rounded-lg object-cover mb-1">
                    @endif
