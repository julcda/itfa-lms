...
<head>
    ...
    <title>{{ config('app.name', school_name()) }}</title>
    ...
    <div class="hidden lg:flex lg:w-[44%] xl:w-[40%] flex-col relative overflow-hidden"
        ...
        <div class="relative flex flex-col flex-1 px-10 py-12">
            ...
            <div class="flex items-center gap-4 mb-14">
                @if($schoolLogo)
                    <img src="{{ Storage::disk('public')->url($schoolLogo) }}"
                         alt="logo"
                ...
                @else
                    <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-black text-lg shrink-0"
                        ...
                        {{ strtoupper(substr($shortName, 0, 2)) }}
                    </div>
                @endif
                <div>
                    <div class="text-white font-bold text-[17px] leading-snug">{{ school_name() }}</div>
                    <div class="text-emerald-400 text-sm mt-0.5">{{ $schoolTagline }}</div>
                </div>
            </div>
            ...
            <div class="flex-1 flex flex-col justify-center">
                ...
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                         style="background:{{ $f['clr'] }}22">
                        <svg class="w-4 h-4" style="color:{{ $f['clr'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $f['path'] }}"/>
                        </svg>
                    </div>
                    <span class="text-slate-300 text-sm font-medium">{{ __($f['lbl']) }}</span>
                </div>
                @endforeach
            </div>
            ...
            <div class="flex items-center gap-2 mt-8">
                <a href="{{ route('locale.switch','en') }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition
                          {{ app()->getLocale()==='en' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/10' }}">
                    EN
                </a>
                <a href="{{ route('locale.switch','ar') }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-semibold transition
                          {{ app()->getLocale()==='ar' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/10' }}">
                    عربي
                </a>
            </div>
        </div>
    ...
    <div class="flex-1 flex flex-col min-h-screen"
        ...
        <div class="lg:hidden flex items-center justify-between px-5 py-4 bg-white shadow-sm border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                @endif
                <span class="font-bold text-gray-800 text-sm truncate max-w-[180px]">{{ school_name() }}</span>
            </div>
            <div class="flex gap-1">
                <a href="{{ route('locale.switch','en') }}"
                   class="px-2 py-1 rounded-md text-xs font-semibold transition
                          {{ app()->getLocale()==='en' ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-gray-700' }}">EN</a>
                <a href="{{ route('locale.switch','ar') }}"
                   class="px-2 py-1 rounded-md text-xs font-semibold transition
                          {{ app()->getLocale()==='ar' ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-gray-700' }}">عربي</a>
            </div>
        </div>
        ...
        <div class="text-center pb-6 text-xs text-gray-400 px-4">
            &copy; {{ date('Y') }} {{ school_name() }} &mdash; {{ $schoolTagline }}
        </div>
    </div>
</div>
